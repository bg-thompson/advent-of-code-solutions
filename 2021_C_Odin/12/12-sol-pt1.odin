// My solution in Odin to Pt 1 of the Day 12 2021 'Advent of Code' challenge.
//
...
// Practice data
//FILENAME     :: "test12-small.txt" // Returned 10 (correct)
//FILENAME     :: "test12.txt" // Returned 226 (correct)
		...
		if gMat[current_node][i] != 0 {
			...
			switch vtype {
				case .END:
					temp_total += 1
				case .SMALL:
					// Check to see if the cave has already been visited.
					prev_visit := false
					for j in 0..<ipath.length {
						if ipath.visited[j] == i { prev_visit = true}
					}
					if !prev_visit {
...
main :: proc() {
	...
	*/
	// Pt 1: Compute total number of paths from start to exit which
	// go through any small cave at most once.
	// Create path starting at START, and replace START with small
	start_index : u8
